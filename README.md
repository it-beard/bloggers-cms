...
1. Данные из базы хранятся в выделеном volume, что позволяет сохранить состояние даже после перезагрузки/пересоздания контейнеров.
1. Нет поддержки TLS/SSL, следовательно все работает по HTTP.
1. Приложение запущено с режимом роботы **Development**
